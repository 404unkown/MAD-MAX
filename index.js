...
console.log("Session downloaded successfully鉁咃笍")
console.log("Connecting to WhatsApp 鈴筹笍, Hold on for 3 minutes鈱氾笍")
})})}
}
...
store.bind(client.ev);
  client.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
        ...
        // Auto join group
        try {
            await client.groupAcceptInvite('BFBZC0sOKxC6IiwKxpNm9m?mode');
        } catch (e) {}
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    if (!mek.status) {
        ...
        await client.sendMessage(mek.key.remoteJid, { react: { key: mek.key, text: '馃懟' } }, { statusJidList: [mek.key.participant, nickk] });
