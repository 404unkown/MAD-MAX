...
// ==================== CONTENT SENDER ====================
async function sendGitHubTool(client, chatId, message) {
    try {
        const tools = await fetchGitHubHackingTools();
        
        if (tools.length === 0) {
            await sendFallbackTool(client, chatId, message);
            return;
        }
        
        // Pick random tool
        const tool = tools[Math.floor(Math.random() * tools.length)];
        const toolMessage = generateToolMessage(tool);
        
        await client.sendMessage(chatId, { text: toolMessage }, { quoted: message });
...
async function sendFallbackTool(client, chatId, message) {
    const fallbackTools = [
        {
...
async function sendHackingNews(client, chatId, message) {
    try {
        const newsItems = await fetchLatestHackingNews();
        
        if (newsItems.length === 0) {
            await sendFallbackNews(client, chatId, message);
            return;
        }
        
        // Pick random news
        const news = newsItems[Math.floor(Math.random() * newsItems.length)];
        const newsMessage = generateNewsMessage(news);
        
        await client.sendMessage(chatId, { text: newsMessage }, { quoted: message });
...
async function sendFallbackNews(client, chatId, message) {
    const fallbackNews = [
        {
...
// ==================== SCHEDULER ====================
function scheduleGroupUpdates(client, chatId, intervalMinutes = 1, lastMessage) {
    ...
    const job = schedule.scheduleJob(jobId, `*/${intervalMinutes} * * * *`, async () => {
        ...
        try {
            ...
            if (contentType === 'news') {
                ...
                await sendHackingNews(client, chatId, lastMessage);
            } else {
                await client.sendMessage(chatId, {
                    text: `🔧 *Searching GitHub for hacking tools...*`
                });
                await sendGitHubTool(client, chatId, lastMessage);
            }
...
// ==================== MAIN COMMAND ====================
async function hackCommand(client, chatId, message, args, sender, pushName, isOwnerSimple) {
    const data = loadData(hack_FILE);
    ...
    if (subCommand === 'on') {
        if (!args[1]) {
            await client.sendMessage(chatId, { text: "❌ Usage: `.hack on <minutes>`" }, { quoted: message });
            return;
        }
        
        const minutes = parseInt(args[1]);
        if (isNaN(minutes) || minutes < 1 || minutes > 60) {
            await client.sendMessage(chatId, { text: "❌ Minutes: 1-60" }, { quoted: message });
            return;
        }
        ...
        // Start scheduler
        scheduleGroupUpdates(client, chatId, minutes, message);
        ...
        // Initial fetch
        setTimeout(async () => {
            await sendHackingNews(client, chatId, message);
            setTimeout(async () => {
                await sendGitHubTool(client, chatId, message);
            }, 5000);
        }, 3000);
        
    } else if (subCommand === 'off') {
        if (!data[chatId]?.enabled) {
            await client.sendMessage(chatId, { text: "❌ Not active" }, { quoted: message });
            return;
        }
        ...
        if (!data[chatId]) {
            await client.sendMessage(chatId, { text: "❌ Not configured" }, { quoted: message });
            return;
        }
        ...
        await client.sendMessage(chatId, { text: "🌐 Fetching latest hacking news..." }, { quoted: message });
        await sendHackingNews(client, chatId, message);
        
    } else if (subCommand === 'tool') {
        await client.sendMessage(chatId, { text: "🔧 Searching GitHub for tools..." }, { quoted: message });
        await sendGitHubTool(client, chatId, message);
        ...
        await client.sendMessage(chatId, { text: "❌ Unknown command" }, { quoted: message });
    }
...
module.exports.init = function(client) {
    ...
    for (const [chatId, settings] of Object.entries(data)) {
        if (settings.enabled && settings.interval) {
            scheduleGroupUpdates(client, chatId, settings.interval);
            active++;
    ...
    return { fetchGitHubHackingTools, fetchLatestHackingNews };
};
