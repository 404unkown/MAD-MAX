            ...
            Object.entries(data).forEach(([chatId, timerData]) => {
                ...
                if (timeLeft > 0) {
                    ...
                    const timer = setTimeout(async () => {
                        try {
                            // We need to get the client instance
                            // This is a limitation - we need to pass client properly
                            console.log(`⏰ Timer expired for ${chatId} (loaded from storage)`);
                            // The actual execution will need to be handled by the main command
                        } catch (error) {
                    ...
                    console.log(`⏰ Restored timer for ${chatId}: ${Math.round(timeLeft/60000)}min left (will ${timerData.targetSetting === 'announcement' ? 'close' : 'open'})`);
                } else {
