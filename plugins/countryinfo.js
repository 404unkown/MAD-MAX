const axios = require('axios');
const { channelInfo } = require('../lib/messageConfig');

function getFlagEmoji(countryCode) {
    if (!countryCode || countryCode.length !== 2) return "🏳️";
    return countryCode
        .toUpperCase()
        .split('')
        .map(letter => String.fromCodePoint(letter.charCodeAt(0) + 127397))
        .join('');
}

async function countryinfoCommand(client, chatId, message, args, sender, pushName, isOwnerSimple) {
    try {
        let query = args.join(' ').trim();
        
        if (!query) {
            const helpText = `
╭─❖ *🌍 COUNTRY INFORMATION* ❖─
│
├─ *Usage:* .country <name/code>
├─ *Examples:*
│  ├─ .country USA
│  ├─ .country Japan
│  ├─ .country IN
│  ├─ .country 44
│  └─ .country Germany
│
├─ *Information includes:*
│  ├─ Basic details
│  ├─ Capital & region
│  ├─ Population & area
│  ├─ Currency & languages
│  ├─ Timezones
│  └─ Flag & calling codes
│
╰─➤ _Get detailed country information_`;

            await client.sendMessage(chatId, {
                text: helpText,
                ...channelInfo
            }, { quoted: message });
            return;
        }
        
        // Send processing reaction
        await client.sendMessage(chatId, { 
            react: { text: '⏳', key: message.key } 
        });
        
        try {
            // Try to get country by name or code
            let response;
            
            if (/^\d+$/.test(query)) {
                // Search by numeric country code
                const countries = await axios.get('https://restcountries.com/v3.1/all');
                const country = countries.data.find(c => {
                    return c.idd && c.idd.root && query === c.idd.root.replace('+', '');
                });
                
                if (country) {
                    response = { data: [country] };
                } else {
                    // Try v2 API
                    const v2Countries = await axios.get('https://restcountries.com/v2/all');
                    const v2Country = v2Countries.data.find(c => 
                        c.callingCodes && c.callingCodes.includes(query)
                    );
                    
                    if (v2Country) {
                        response = { data: [v2Country] };
                    }
                }
            } else {
                // Search by name or alpha code
                try {
                    response = await axios.get(`https://restcountries.com/v3.1/name/${encodeURIComponent(query)}`);
                } catch (error) {
                    // Try by alpha code
                    response = await axios.get(`https://restcountries.com/v3.1/alpha/${query.toUpperCase()}`);
                }
            }
            
            if (!response || !response.data || response.data.length === 0) {
                await client.sendMessage(chatId, {
                    text: `❌ *Country not found*\n"${query}" not found in database.`,
                    ...channelInfo
                }, { quoted: message });
                await client.sendMessage(chatId, { 
                    react: { text: '❌', key: message.key } 
                });
                return;
            }
            
            const country = response.data[0];
            
            // Format country information
            const flag = getFlagEmoji(country.cca2);
            const name = country.name?.common || 'Unknown';
            const nativeName = country.name?.nativeName 
                ? Object.values(country.name.nativeName)[0].common 
                : name;
            
            const countryInfo = `
${flag} *${name}* (${nativeName})
━━━━━━━━━━━━━━━
📌 *Code:* ${country.cca2 || ''}${country.cca3 ? ` / ${country.cca3}` : ''}
📞 *Calling:* ${country.idd?.root || ''}${country.idd?.suffixes?.[0] || ''}
🏛️ *Capital:* ${country.capital?.[0] || 'N/A'}
🗺️ *Region:* ${country.region || 'N/A'}${country.subregion ? ` (${country.subregion})` : ''}

*📊 Demographics:*
👥 *Population:* ${new Intl.NumberFormat().format(country.population || 0)}
📏 *Area:* ${new Intl.NumberFormat().format(Math.round(country.area || 0))} km²
📍 *Coordinates:* ${country.latlng?.join(', ') || 'N/A'}

*💰 Economy:*
💵 *Currency:* ${country.currencies ? Object.values(country.currencies).map(c => `${c.name} (${c.symbol || ''})`).join(', ') : 'N/A'}

*🌐 Languages:*
🗣️ *Languages:* ${country.languages ? Object.values(country.languages).join(', ') : 'N/A'}

*🕐 Timezone:*
⏰ *Timezones:* ${country.timezones ? country.timezones.slice(0, 3).join(', ') : 'N/A'}

━━━━━━━━━━━━━━━
${country.tld?.[0] ? `🌐 *Domain:* ${country.tld[0]}` : ''}
${country.car?.side ? `🚗 *Drives on:* ${country.car.side}` : ''}
${country.independent !== undefined ? `🏳️ *Independent:* ${country.independent ? 'Yes' : 'No'}` : ''}
${country.unMember !== undefined ? `🇺🇳 *UN Member:* ${country.unMember ? 'Yes' : 'No'}` : ''}`;
            
            await client.sendMessage(chatId, {
                text: countryInfo,
                ...channelInfo
            }, { quoted: message });
            
            await client.sendMessage(chatId, { 
                react: { text: '✅', key: message.key } 
            });
            
        } catch (error) {
            console.error('Country info error:', error);
            await client.sendMessage(chatId, {
                text: `❌ *Failed to fetch country information*\nError: ${error.message}`,
                ...channelInfo
            }, { quoted: message });
            await client.sendMessage(chatId, { 
                react: { text: '❌', key: message.key } 
            });
        }
        
    } catch (error) {
        console.error('Countryinfo command error:', error);
        await client.sendMessage(chatId, {
            text: `❌ *Unexpected error*\n${error.message}`,
            ...channelInfo
        }, { quoted: message });
        await client.sendMessage(chatId, { 
            react: { text: '❌', key: message.key } 
        });
    }
}

module.exports = countryinfoCommand;