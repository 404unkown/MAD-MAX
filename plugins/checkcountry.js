...
const COUNTRY_CODES_DB = {
    ...
    '254': [{ name: 'Kenya', code: 'KE', capital: 'Nairobi', region: 'Africa' }],
...
module.exports = async function checkcountryCommand(client, chatId, message, args, sender, pushName, isOwner) {
    try {
        // Get the code from args
        let code = args[0]?.trim() || '';

        // Check if code is provided
        if (!code) {
            const helpText = `╭─❖ *COUNTRY CODE CHECKER* ❖─
│
├─ *Usage:* .check <code>
├─ *Examples:*
│  ├─ .check 254 
...
╰─➤ _Requested by: ${pushName}_`;
        ...
        if (matchingCountries.length > 0) {
            ...
            for (const country of matchingCountries) {
                ...
                const countryInfo = `${flag} *${country.name}* (${country.code})
├─ 📞 *Calling Code:* +${code}
...
${country.languages && country.languages !== 'N/A' ? `└─ 🗣️ *Languages:* ${country.languages}` : ''}`;
            ...
            const resultText = `╭─❖ *COUNTRY CODE RESULTS* ❖─
│
│ 📞 *Country Code:* +${code}
